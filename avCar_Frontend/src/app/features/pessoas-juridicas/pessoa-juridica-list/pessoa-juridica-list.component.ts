...
export class PessoaJuridicaListComponent implements OnInit {
  displayedColumns = ['id', 'razaoSocial', 'cnpj', 'telefone', 'email', 'actions'];
  ...
  load(): void {
    ...
    this.service.getAll(this.pageIndex, this.pageSize).subscribe({
      ...
      error: () => {
        this.snackBar.open('Erro ao carregar pessoas jurídicas', 'Fechar', { duration: 3000 });
        this.loading = false;
      },
    });
  }
  ...
  openForm(item?: PessoaJuridica): void {
    const dialogRef = this.dialog.open(PessoaJuridicaFormComponent, {
      width: '600px',
      data: item ? { ...item } : null,
    });
  ...
  delete(item: PessoaJuridica): void {
    this.dialog.open(ConfirmDialogComponent, {
      width: '420px',
      data: {
        title: 'Confirmar exclusão',
        message: `Deseja excluir "${item.razaoSocial}"? Esta ação não pode ser desfeita.`,
        confirmText: 'Excluir',
      ...
      this.service.delete(item.id!).subscribe({
        next: () => {
          this.snackBar.open('Pessoa jurídica excluída', 'Fechar', { duration: 3000 });
          this.load();
        },
        error: () => {
